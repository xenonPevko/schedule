...
async def select_group(message: Message, state: FSMContext):
    # Проверяем, не находимся ли мы в процессе добавления расписания
    current_state = await state.get_state()
    if current_state and current_state.startswith("AddLessonStates"):
        return  # Игнорируем, если идёт добавление расписания
